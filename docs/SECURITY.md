...
|:--|:--|:--|
| **`JWT_SECRET` has a working default** | `docker-compose.yml` falls back to `dev-only-insecure-secret-change-me` so that one-command start works with no `.env`. That one value signs access tokens **and** keys the presigned-artifact HMAC (`storage/store.py`), so anyone who knows the published default can forge a session **and** mint a download token for any storage key. It is the single highest-impact item on this page | Refuse to boot on the default when `DEMO_MODE=false`, or generate one in `make up`. It is already the second item on the [pre-deployment checklist](OPERATIONS.md#8-before-a-shared-deployment) — this row says what happens if that line is skipped |
