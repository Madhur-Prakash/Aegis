...
make up           # fills placeholder secrets, then compose up -d --wait
...
```
$ make up
ensure-env: generated JWT_SECRET (32 bytes) - any existing session is now invalid
```

> [!CAUTION]
> This exists because `JWT_SECRET` signs access tokens **and** keys the HMAC on the presigned
> artifact URLs. `docker-compose.yml` carries a fallback so a clean clone starts with one command —
> but that fallback is a constant published in this repository, and anyone who reads it could forge
> a session for any user and mint a download token for any storage key.

It is deliberately a **no-op once a real value is present** — regenerating on every `make up` would
sign every user out on every restart. A value counts as a placeholder only if it is empty, is the
published compose fallback, shouts `CHANGE_ME`, or is shorter than 32 characters. Add a name to
`KEYS` in that script and it is filled the same way.

Running `docker compose up` directly bypasses this, which is the one case
[Security §8](SECURITY.md#8-not-built-and-named-as-such) still names.
...
| `JWT_SECRET` | generated by `make up` | Filled with `openssl rand -hex 32` on first run by [`ensure-env`](#secrets-are-filled-before-anything-starts), then left alone. Rotating it invalidates every session. It also keys the presigned-artifact HMAC — see [Security §8](SECURITY.md#8-not-built-and-named-as-such). |
...
- [ ] `JWT_SECRET` is a real value, not the published Compose fallback — `make up` fills it
      automatically, so this is only a manual step if you started with `docker compose up`
